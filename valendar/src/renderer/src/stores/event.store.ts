...
export const useEventStore = defineStore('event', () => {
  ...
  async function fetchEvents(params?: EventQueryParams): Promise<void> {
    log.info('fetchEvents called')
    loading.value = true
    error.value = null
    try {
      if (!window.api?.event?.getAll) {
        log.warn('window.api.event.getAll 不可用')
        return
      }
      events.value = await window.api.event.getAll(params)
      log.info('fetchEvents loaded', events.value.length, 'events')
    } catch (err) {
  ...
  async function createEvent(input: EventInput): Promise<CalendarEvent | null> {
    log.info('createEvent called:', input.title)
    loading.value = true
    error.value = null
    try {
      ...
      log.info('Calling IPC event:create...')
      const event = await window.api.event.create(input)
      log.info('IPC returned:', event?.id, event?.title)
      events.value.push(event)
      return event
    } catch (err) {
      const errMsg = String(err)
      log.error('createEvent failed:', errMsg)
      error.value = errMsg
      return null
    } finally {
      loading.value = false
  ...
  async function deleteEvents(ids: string[]): Promise<number> {
    ...
    try {
      ...
      error.value = String(err)
      log.error('Failed to delete events:', err)
      return 0
    } finally {
      loading.value = false
    }
  ...
  async function getEventsByDateRange(
    ...
    try {
      ...
      error.value = String(err)
      log.error('Failed to get events by date range:', err)
      return []
    } finally {
      loading.value = false
    }
  ...
  async function getEventsByCategory(category: string): Promise<CalendarEvent[]> {
    ...
    try {
      ...
      error.value = String(err)
      log.error('Failed to get events by category:', err)
      return []
    } finally {
      loading.value = false
    }
